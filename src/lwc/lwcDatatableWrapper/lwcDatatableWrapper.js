...
export default class LwcDatatableWrapper extends LightningElement {
  ...
  get illustrationMessage() {
    if (this.objectNotFound)
      return objectNotFound.replace("{0}", this.objectApiName);
    if (this.fieldsetNotFound)
      return fieldsetNotFound.replace("{0}", this.objectApiName);
  ...
  async handleInitialLoad() {
    ...
    const errorFn = (error) => {
      this.objectNotFound =
        error.body?.message ===
        objectNotFound.replace("{0}", this.objectApiName);
      this.fieldsetNotFound =
        error.body?.message ===
        fieldsetNotFound.replace("{0}", this.objectApiName);
  ...
  showSuccessToast(message) {
    new Toast(Toast.emptyString, message, Toast.variants.success).dispatch();
  }

  showErrorToast(error) {
    new Toast(Toast.emptyString, error, Toast.variants.error).dispatch();
  }
}
